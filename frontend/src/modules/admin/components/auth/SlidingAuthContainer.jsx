...
import { Facebook, Linkedin, ArrowLeft, ShieldCheck, Eye, EyeOff } from 'lucide-react';
...
export default function SlidingAuthContainer({ initialMode = 'signin' }) {
    ...
    const handleRegister = async (e) => {
        ...
        try {
            const data = await authService.adminRegister(regName.trim(), regEmail.trim(), regPassword, regSecret.trim());
            login(data.token, data.user);
            navigate('/admin/dashboard');
        } catch (err) {
    ...
    // Forgot password — still UI-only (needs email backend)
